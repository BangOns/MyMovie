...
function ListDetailsFilmPopular({ ids }) {
  const { id } = useParams();
  ...
  const { data, isLoading } = useQuery(
    ...
    () => {
      return axios.get(
        `https://api.themoviedb.org/3/discover/movie?api_key=df3bdd5a174cac305c5d71d51733fff7&sort_by=popularity.desc&with_genres=${id}&page=${ids}`
      );
    },
    {
      select: (data) => {
            ...
            <Fragment key={index}>
              <div className="FilmSearch">
                <div
                  ...
                  onClick={() => navigate(`/detailsFilms/${genre.id}`)}
                >
                  <img
                ...
                <div className="nameFilmSearch">
                  <p>{genre.title ? genre.title : "Null"}</p>
                </div>
              </div>
            </Fragment>
          );
...
export default ListDetailsFilmPopular;
