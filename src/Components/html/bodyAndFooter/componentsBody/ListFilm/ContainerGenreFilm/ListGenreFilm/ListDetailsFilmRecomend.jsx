...
function ListDetailsFilmRecomend({ ids }) {
  const { id } = useParams();
  ...
  const { data, isLoading } = useQuery(
    ...
    () => {
      return axios.get(
        `https://api.themoviedb.org/3/movie/top_rated?api_key=df3bdd5a174cac305c5d71d51733fff7&with_genres=${id}&page=${ids}`
      );
    },
    {
      select: (data) => {
...
export default ListDetailsFilmRecomend;
